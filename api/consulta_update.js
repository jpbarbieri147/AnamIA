// /api/consulta_update.js — Renomeia uma consulta (define titulo) do medico.
export default async function handler(req, res) {
  ...
  try {
    const { access_token, id, titulo } = req.body || {};
    if (!access_token) return res.status(401).json({ error: 'Missing access_token' });
    if (!id) return res.status(400).json({ error: 'Missing id' });
    const tituloLimpo = (titulo !== undefined && titulo !== null) ? String(titulo).trim() : '';
    ...
    return res.status(200).json({ id: id, titulo: tituloLimpo });
  } catch (e) {
