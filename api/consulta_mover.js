// /api/consulta_mover.js — Move uma consulta para outra pasta (atualiza paciente_id).
// Valida token + propriedade antes de atualizar.
export default async function handler(req, res) {
  ...
  try {
    const { access_token, id, paciente_id } = req.body || {};
    if (!access_token) return res.status(401).json({ error: 'Missing access_token' });
    if (!id) return res.status(400).json({ error: 'Missing consulta id' });
    // paciente_id pode ser null (mover para "Sem pasta")
    ...
    // Atualiza paciente_id filtrando por id E medico_id (garante propriedade)
    ...
    const updResp = await fetch(updUrl, {
      ...
      body: JSON.stringify({ paciente_id: paciente_id || null })
    });

    if (!updResp.ok) {
      const errText = await updResp.text();
      return res.status(500).json({ error: 'Move failed: ' + errText });
    }
    const updated = await updResp.json();
    if (!updated || updated.length === 0) return res.status(404).json({ error: 'Consulta nao encontrada' });
    return res.status(200).json({ ok: true, id: id, paciente_id: paciente_id || null });
  } catch (e) {
